...
def get_table_config(flag):
    flags = {3: 'DEPRECATED_MASK(3)'}
    try:
        return flags[flag]
    except KeyError:
        return 'UnknownFlag(%s)' % flag
...
def get_meter(meter):
    meters = {4294901760: 'OFPM_MAX(0xFFFF0000)',
              4294967293: 'OFPM_SLOWPATH(0xFFFFFFFD)',
              4294967294: 'OFPM_CONTROLLER(0xFFFFFFFE)',
              4294967295: 'OFPM_ALL(0xFFFFFFFF)'}
    try:
        return meters[meter]
    except KeyError:
        return '%s' % meter
...
def get_meter_mod_command(command):
    commands = {0: 'OFPMC_ADD',
                1: 'OFPMC_MODIFY',
                2: 'OFPMC_DELETE'}
    try:
        return commands[command]
