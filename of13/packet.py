...
class ofp_multipart_request(ofp_header):
    ...
    def instantiate(self, *args):
        if self.stat_type in [0, 3, 7, 8, 11, 13]:
            ...
            self.body = ofp_meter_multipart_request(*args)
...
# ofp_multipart_req aux classes


class ofp_flow_stats_request:

    def __init__(self, table_id, pad, out_port, out_group, pad2, cookie, cookie_mask, match):
...
class ofp_queue_stats_request:
    ...
    def __init__(self, port_no, queue_id):
        self.port_no = port_no
...
class  ofp_group_stats_request:
    ...
    def __init__(self, group_id, pad):
        self.group_id = group_id
        self.pad = pad


class ofp_meter_multipart_request:

    def __init__(self, meter_id, pad):
...
class ofp_table_features:
    ...
    def __init__(self, length, table_id, pad, name, metadata_match, metadata_write, config, max_entries):
        self.length = length
        self.table_id = table_id
        self.pad = pad
        self.name = name
        ...
        self.table_feature_prop = []


class ofp_experimenter_multipart_header:

    def __init__(self, experimenter, exp_type):
...
class ofp_queue_prop_experimenter:
    def __init__(self):
        self.prop_header = ofp_queue_prop_header()
